...
public class CaesarCoder {
    ...
    void caesarCoder(String text, int key) {
        try {
            for (int i = 0; i < text.length(); i++) {
                char currentChar = text.charAt(i);
                if (Alphabet.EN_BIG.contains(currentChar)) {
                    int index = Alphabet.EN_BIG.indexOf(currentChar);
                    int newIndex = (index - key + Alphabet.EN_BIG.size()) % Alphabet.EN_BIG.size();
                    ...
                    int newIndex = (index - key + Alphabet.EN_SMALL.size()) % Alphabet.EN_SMALL.size();
